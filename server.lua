...
local function validateConfig()
    if maxForecast <= 0 then
        log("error", "maxForecast must be greater than 0")
        maxForecast = 1
    end
    if syncDelay <= 0 then
        log("error", "syncDelay must be greater than 0")
        syncDelay = 1000
    end
end
...
local function setWeather(weather, transition, freeze, permSnow)
    if not contains(Config.weatherTypes, weather) then
        log("error", "Unknown weather type: " .. weather)
        return
    end
...
local function setSyncDelay(delay)
    if delay <= 0 then
        log("error", "Sync delay must be greater than 0")
        return
    end
    syncDelay = delay
end
...
AddEventHandler('txAdmin:events:scheduledRestart', function(eventData)
    if eventData.secondsRemaining == 1800 then
        ...
        TriggerEvent('weathersync:setWeather', "overcastdark", 10.0, true, false)
        Citizen.Wait(9000)
    elseif eventData.secondsRemaining == 600 then
        print("10 Mins till Storm")
        TriggerEvent('weathersync:setWeather', "hurricane", 10.0, true, false)
        Citizen.Wait(9000)
    elseif eventData.secondsRemaining == 300 then
        print("5 Min till Storm")
        TriggerEvent('weathersync:setWeather', "thunderstorm", 10.0, true, false)
        Citizen.Wait(9000)
    end
end)
